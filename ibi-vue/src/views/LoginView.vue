...
const doLogin = async () => {
    ...
    if (success)
        toast.add({severity: 'success', summary: 'Success', detail: 'Check your email for the login link'})
    else
        ...
        <Card class="mt-3 w-1/3">
            ...
            <template #content>
                <div class="flex items-center gap-8 flex-col">
                    ...
                    <Button :disabled="!email || isLoading" label="Request Link" icon="pi pi-sign-in" rounded class="text-center w-48" type="submit" @click="doLogin" />
                </div>
            </template>
        </Card>
    </div>
